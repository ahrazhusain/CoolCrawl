...
def sessionInfoGen(xmldoc,root,sheet1):
    ...
    timeBackup = int(itemlist[0].attributes['tracker-time'].value)
    ...
    for s in itemlist :
        ...
        timeFound = int(s.attributes['tracker-time'].value)
        lineInQuestion=int(s.attributes['line'].value)
        #print "Found at", timeFound
        
        if(timeFound==timeBackup):
            s.attributes['line'].value
            meanPupil+=(float(s.attributes['left-pupil-diameter'].value)+float(s.attributes['right-pupil-diameter'].value))/2.0
            t=t+1
            eventsT=0
        else:
            ...
            for k in root.findall('InteractionEvent'):
                ...
                if timeToFind in r:
                    ...
                    if(eventsT==0):
                        ...
                        eventsT=1
            """if(wasAction==1):
                print "Impression at time: ", timeBackup, "at line:",lineInQuestion," of degree ", Pdegree," was action: ", wasAction"""
            sheet1.write(n, 0, timeBackup)
            sheet1.write(n, 1, lineInQuestion)
            sheet1.write(n, 2, Pdegree)
            sheet1.write(n, 3, wasAction)
            n+=1
            wasAction=0
                ...
                for mFile in mFilesFound:
                    ...
                    if mP.lower() in mFile.lower():
                        ...
                        if xmldoc.getElementsByTagName('itrace-records'):
                            ...
                            sheet1.write(0, 1, "line")
                            sheet1.write(0, 2, "Degree")
                            sheet1.write(0, 3, "Action?")
    
                            sheet1=sessionInfoGen(xmldoc,root,sheet1)
                            
                            bookName="session_02_Data/User"+str(id)+"-task"+str(task)+".xls"
                            book.save(bookName)
